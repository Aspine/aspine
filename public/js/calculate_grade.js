	...
	if (Object.keys(categories).length === 0) {
		...
		for (let i = 0; i < assignments.length; i++) {
			if (!isNaN(assignments[i].score)) {
				...
				categoryScores[assignments[i].category] += parseFloat(
					assignments[i].score
				);
				categoryMaxScores[assignments[i].category] += parseFloat(
		...
		for (let category in categories) {
			if (categoryMaxScores[category] === 0) {
				...
				categoryGrades[category] =
					(0.0 + categoryScores[category]) /
					categoryMaxScores[category];
				categoryPercent +=
					((0.0 + categoryScores[category]) /
						categoryMaxScores[category]) *
					parseFloat(categories[category]);
			}
		}
		...
		let totalPercent = totalScore / totalMaxScore;
		let output =
			parseFloat(grade) / 100 + categoryPercent - parseFloat(init_grade);

		return {
			categoryPercent: (
				Math.round(output * Math.pow(10, decimals + 2)) /
				Math.pow(10, decimals)
			).toString(),
			totalPercent: (
				Math.round(totalPercent * Math.pow(10, decimals + 2)) /
				Math.pow(10, decimals)
			).toString(),
			categoryScores,
			categoryMaxScores,
...
let getCategoryDisplay = function (gradeInfo, computingClassData) {
	...
	let weightsArray = Object.values(computingClassData.categories).map(
		(weight) => weight * 100 + '%'
	);
	let scoresArray = Object.values(gradeInfo.categoryScores).map((score) =>
		parseFloat(score) != 0 ? score : 'Ungraded'
	);
	let maxScoresArray = Object.values(gradeInfo.categoryMaxScores).map(
		(maxScore) => (parseFloat(maxScore) != 0 ? maxScore : 'Ungraded')
	);
	let gradesArray = Object.values(gradeInfo.categoryGrades).map((grade) =>
...
function GPAType() {
	...
	let selection =
		$('#gpa_select option')[selectElem.prop('selectedIndex')].value;
		...
		if (quarterData.GPA.outOfFour != quarterData.calcGPA.outOfFour) {
			$('#current, #current_gpa, #init_gpa').replace_text(
				'Current Quarter Unweighted: ' +
					...
					'<br> Calculated: ' +
					currentTableData.terms.current.calcGPA.outOfFour.toFixed(2)
			);
			for (let i = 1; i <= 4; i++) {
				$(`#q${i}, #q${i}_gpa`).replace_text(
					'Q' +
						i +
						' Unweighted: ' +
						currentTableData.terms['q' + i].GPA.outOfFour.toFixed(
							2
						) +
			...
			for (let i = 1; i <= 4; i++) {
				$(`#q${i}, #q${i}_gpa`).replace_text(
					'Q' +
						i +
						' Unweighted: ' +
						currentTableData.terms['q' + i].GPA.outOfFour.toFixed(2)
				);
			}
			selectedElem.replace_text(
				quarterName +
		...
		if (quarterData.GPA.outOfFive != quarterData.calcGPA.outOfFive) {
			$('#current, #current_gpa, #init_gpa').replace_text(
				'Current Quarter Weighted: ' +
					...
					'<br> Calculated: ' +
					currentTableData.terms.current.calcGPA.outOfFive.toFixed(2)
			);
			for (let i = 1; i <= 4; i++) {
				$(`#q${i}, #q${i}_gpa`).replace_text(
					'Q' +
						i +
						' Weighted: ' +
						currentTableData.terms['q' + i].GPA.outOfFive.toFixed(
							2
						) +
						(currentTableData.terms['q' + i].calcGPA
							? '<br> Calculated: ' +
			...
			for (let i = 1; i <= 4; i++) {
				$(`#q${i}, #q${i}_gpa`).replace_text(
					'Q' +
						i +
						' Weighted: ' +
						currentTableData.terms['q' + i].GPA.outOfFive.toFixed(2)
				);
			}
			selectedElem.replace_text(
				quarterName +
		...
		if (quarterData.GPA.percent != quarterData.calcGPA.percent) {
			$('#current, #current_gpa, #init_gpa').replace_text(
				'Current Quarter GPA: ' +
					...
					'<br> Calculated: ' +
					currentTableData.terms.current.calcGPA.percent.toFixed(2)
			);
			for (let i = 1; i <= 4; i++) {
				$(`#q${i}, #q${i}_gpa`).replace_text(
					'Q' +
						i +
						' GPA: ' +
						currentTableData.terms['q' + i].GPA.percent.toFixed(2) +
						(currentTableData.terms['q' + i].calcGPA
							? '<br> Calculated: ' +
								currentTableData.terms[
									'q' + i
