...
class Snackbar {
	...
	constructor(
		text,
		options = {
			destroyWhenButtonClicked: true,
			destroyWhenBodyClicked: true,
			timeoutFunction: () => {},
			timeoutEndFunction: 'destroy'
		}
	) {
		this.text = text;
	...
	make() {
		...
		//adds color if given
		if (this.color) snackbarNode.style.backgroundColor = this.color;

		// sets the body onclick listener which just destroys it by default
		const bodyOnClickFunction =
			this.bodyClick !== undefined ? () => this.bodyClick() : () => {};
		const destroyFromBody = this.destroyWhenBodyClicked
			? () => this.destroy()
			: () => {};
		...
		if (this.textColor) textNode.style.color = this.textColor;
		...
		if (this.buttonText && this.buttonClick) {
			...
			//colors the text if necessary
			if (this.textColor) buttonTextNode.style.color = this.textColor;

			if (this.color) buttonNode.style.backgroundColor = this.color;

			// adds the text node
			buttonNode.appendChild(buttonTextNode);

			// sets the button onclick listener which runs the given funtion and destroys the snackar by default
			const destroyFromButton = this.destroyWhenButtonClicked
				? () => this.destroy()
				: () => {};
	...
	destroy() {
		...
		const finalizeDeletion = function () {
			...
			delete Snackbar.snackbarIDs[
				Snackbar.snackbarIDs.indexOf(snackbar.id)
			];
