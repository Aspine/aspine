...
let setup_quarter_dropdown = function () {
	...
	document.querySelector('.gpa_select-selected').textContent =
		document.querySelector('#gpa_select').options[
...
let dayOfWeek_onclick = function () {
	...
	for (let i = 0; i < select.length; i++) {
		if (select.options[i].innerHTML === this.innerHTML) {
			...
			document.getElementById('day_select_div').innerHTML =
				this.innerHTML;
		...
		if (child.querySelector('.tooltiptext') === null) {
			...
			if (child.hasAttribute('tooltip-margin')) {
				tooltiptext.style.marginLeft =
					child.getAttribute('tooltip-margin');
...
let anyEdited = function () {
	let termsEdited = currentTableData.terms[currentTerm].classes.map(
