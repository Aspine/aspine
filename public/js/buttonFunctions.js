...
function replaceAssignmentFromID(oldData, newData, classID) {
	const assignments =
		currentTableData.currentTermData.classes[classID].assignments;

	const index = assignments.findIndex(
		({ assignment_id }) => assignment_id === oldData.assignment_id
	);
...
function removeAssignmentFromID(id, classID) {
	const assignments =
		currentTableData.currentTermData.classes[classID].assignments;
	const newArray = assignments.filter(
...
let editAssignment = function (data) {
	...
	for (
		...
		currentTableData.currentTermData.classes[selected_class_i].assignments[
			...
			currentTableData.currentTermData.classes[selected_class_i]
				.assignments[j].percentage
		);

		if (
			currentTableData.currentTermData.classes[
				selected_class_i
			].assignments[j].category.includes('(')
		) {
			currentTableData.currentTermData.classes[
				selected_class_i
			].assignments[j].category =
				currentTableData.currentTermData.classes[
					selected_class_i
				].assignments[j].category.substring(
					0,
					currentTableData.currentTermData.classes[
						selected_class_i
					].assignments[j].category.indexOf('(') - 1
				);
		}
...
let resetTableData = function () {
	...
	currentTableData.terms[currentTerm] = JSON.parse(
	...
	if (anyEdited()) {
		...
		if (currentTerm === 'current') {
			...
			document.getElementById('gpa_select').options[0].innerHTML =
				'Current Quarter GPA: ' +
				GPA.percent +
				'<br>Calculated GPA: ' +
				calcGPA.percent;
			document.getElementById('gpa_select').options[1].innerHTML =
				'Current Quarter GPA: ' +
				GPA.percent +
				'<br>Calculated GPA: ' +
				calcGPA.percent;
			...
			$('.gpa_select-selected').html(
				'Q' +
					termConverter.indexOf(currentTerm) +
					' GPA: ' +
					GPA.percent +
					'<br>Calculated GPA: ' +
					calcGPA.percent
			...
			document.getElementById('gpa_select').options[
		...
		if (currentTerm === 'current') {
			...
			$('.gpa_select-selected').html(
				'Current Quarter GPA: ' + GPA.percent
			);
...
let updateGradePage = function () {
	let computingClassData =
		currentTableData.currentTermData.classes[selected_class_i];

	let gradeInfo = computeGrade(
	...
	currentTableData.currentTermData.classes[
		selected_class_i
	].calculated_grade = gradeInfo[computingClassData.type];

	currentTableData.currentTermData.classes[selected_class_i].categoryDisplay =
		getCategoryDisplay(gradeInfo, computingClassData);
	...
	assignmentsTable.replaceData(
		currentTableData.currentTermData.classes[
			selected_class_i
		].assignments.filter(({ placeholder }) => !placeholder)
	);

	currentTableData.currentTermData.calcGPA = computeGPA(
		currentTableData.currentTermData.classes
	);
	currentTableData.terms[currentTerm].calcGPA = computeGPA(
		currentTableData.currentTermData.classes
	);
		...
		if (selectedElem.html().includes('GPA')) {
			if (quarterData.GPA.percent !== quarterData.calcGPA.percent) {
				$('#current, #current_gpa, #init_gpa').html(
					'Current Quarter GPA: ' +
						currentTableData.terms.current.GPA.percent.toFixed(2) +
						'<br> Calculated: ' +
						currentTableData.terms.current.calcGPA.percent.toFixed(
							2
						)
				);
				for (let i = 1; i <= 4; i++) {
					$(`#q${i}, #q${i}_gpa`).html(
						'Q' +
							i +
							' GPA: ' +
							currentTableData.terms['q' + i].GPA.percent.toFixed(
								2
							) +
				...
				selectedElem.html(
					quarterName +
						' GPA: ' +
						quarterData.GPA.percent.toFixed(2) +
						'<br> Calculated: ' +
						quarterData.calcGPA.percent.toFixed(2)
				);
				$('#cum, #cum_gpa').html(
			...
			if (quarterData.GPA.outOfFour !== quarterData.calcGPA.outOfFour) {
				$('#current, #current_gpa, #init_gpa').html(
					'Current Quarter Unweighted: ' +
						currentTableData.terms.current.GPA.outOfFour.toFixed(
							2
						) +
						'<br> Calculated: ' +
						currentTableData.terms.current.calcGPA.outOfFour.toFixed(
							2
						)
				);
				for (let i = 1; i <= 4; i++) {
					$(`#q${i}, #q${i}_gpa`).html(
						'Q' +
							...
							currentTableData.terms[
								'q' + i
							].GPA.outOfFour.toFixed(2) +
							(currentTableData.terms['q' + i].calcGPA
								? '<br> Calculated: ' +
									currentTableData.terms[
										'q' + i
									].calcGPA.outOfFour.toFixed(2)
								: '')
					);
				}
				selectedElem.html(
					quarterName +
						' Unweighted: ' +
						quarterData.GPA.outOfFour.toFixed(2) +
						'<br> Calculated: ' +
						quarterData.calcGPA.outOfFour.toFixed(2)
				);
				$('#cum, #cum_gpa').html(
					'Cumulative Unweighted: ' +
						currentTableData.cumGPA.outOfFour.toFixed(2)
				);
			} else {
				$('#current, #current_gpa, #init_gpa').html(
					'Current Quarter Unweighted: ' +
						currentTableData.terms.current.GPA.outOfFour.toFixed(2)
				);
				for (let i = 1; i <= 4; i++) {
					$(`#q${i}, #q${i}_gpa`).html(
						'Q' +
			...
			if (quarterData.GPA.outOfFive != quarterData.calcGPA.outOfFive) {
				$('#current, #current_gpa, #init_gpa').html(
					'Current Quarter Weighted: ' +
						currentTableData.terms.current.GPA.outOfFive.toFixed(
							2
						) +
						'<br> Calculated: ' +
						currentTableData.terms.current.calcGPA.outOfFive.toFixed(
							2
						)
				);
				for (let i = 1; i <= 4; i++) {
					$(`#q${i}, #q${i}_gpa`).html(
						'Q' +
							...
							currentTableData.terms[
								'q' + i
							].GPA.outOfFive.toFixed(2) +
							(currentTableData.terms['q' + i].calcGPA
								? '<br> Calculated: ' +
									currentTableData.terms[
										'q' + i
									].calcGPA.outOfFive.toFixed(2)
								: '')
					);
				}
				selectedElem.html(
					quarterName +
						' Weighted: ' +
						quarterData.GPA.outOfFive.toFixed(2) +
						'<br> Calculated: ' +
						quarterData.calcGPA.outOfFive.toFixed(2)
				);
				$('#cum, #cum_gpa').html(
					'Cumulative Weighted: ' +
						currentTableData.cumGPA.outOfFive.toFixed(2)
				);
			} else {
				$('#current, #current_gpa, #init_gpa').html(
					'Current Quarter Weighted: ' +
						currentTableData.terms.current.GPA.outOfFive.toFixed(2)
				);
				for (let i = 1; i <= 4; i++) {
					$(`#q${i}, #q${i}_gpa`).html(
						'Q' +
	...
	if (prefs.terms) {
		...
		try {
			terms = await Promise.all(
				termConverter.map(async (term) => {
					...
					// Term is selected by user and its data have not been downloaded
						...
						prefs.terms[term] &&
						!currentTableData.imported /* backwards compat */ &&
			...
			currentTableData.currentTermData =
				currentTableData.terms[currentTerm];
			classesTable.setData(currentTableData.currentTermData.classes);
			classesTable.redraw();
		}
	}
...
let importTableData = async function (obj) {
	...
	if (
		...
		return (
			`This JSON file is from Aspine version ${obj.version} and is ` +
		...
		responseCallback(
			{
				...
				recent: {
					...
					recentAttendanceArray:
						obj.recent.recentAttendanceArray || []
				},
	...
	for (const i in tableData) {
		if (!$(`#tableData_select option[value='${i}']`)[0]) {
			...
			$('#tableData_select')[0].insertBefore(
				option,
				$("#tableData_select option[value='import']")[0]
			);
			...
			$('.tableData_select-items')[0].insertBefore(
				div,
				$('#tableData_select-items-import')[0]
			);
		}
	}
	...
	if (currentTableDataIndex === 0) {
		...
		$("#tableData_select option[value='0']")[0].textContent =
			tableData[0].name;
		$('#tableData_select-items-0')[0].textContent = tableData[0].name;
		...
		$("#tableData_select option[value='1']")[1].textContent =
			tableData[1].name;
		$('#tableData_select-items-1')[1].textContent = tableData[1].name;
	}
