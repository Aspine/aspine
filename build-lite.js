...
async function main() {
  ...
  await Promise.all([...dep_mappings.files, ...dep_mappings.directories]
    .map(([endpoint, path]) => fse.copy(
  ...
  await Promise.all((await fsp.readdir(pathLib.join(OUTPUT, "js")))
    .map(file => processJS(pathLib.join("js", file)))
  );
}
  ...
  try {
    ...
    for await (const line of rl) {
      ...
      if ((match = line.match(/^\/\/#include (.*)/))) {
        const path = pathLib.join(__dirname, pathLib.normalize(match[1]));
